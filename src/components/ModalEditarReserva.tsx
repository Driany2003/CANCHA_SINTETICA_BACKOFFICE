...
const ModalEditarReserva: React.FC<ModalEditarReservaProps> = ({
  ...
  const [estados] = useState<EstadoReserva[]>(['pendiente_de_pago', 'pagado_pendiente_confirmacion', 'pagado_confirmado']);
  ...
  // Solo permitir editar si el estado es pendiente_de_pago o pagado_pendiente_confirmacion
  const puedeEditar = reserva.estado === 'pendiente_de_pago' || reserva.estado === 'pagado_pendiente_confirmacion';
  if (!puedeEditar) {
    return ReactDOM.createPortal(
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100]">
        <div className="bg-white dark:bg-[#101f28] rounded-lg shadow-xl max-w-md w-full mx-4 p-6 text-center border border-slate-200 dark:border-slate-600">
          <div className="text-red-600 dark:text-red-400 mb-4">
            <svg className="h-16 w-16 mx-auto" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
            </svg>
          </div>
          <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">No se puede editar</h3>
          <p className="text-slate-600 dark:text-slate-300 mb-4">
            Solo se pueden editar reservas con estado &quot;Pendiente de Pago&quot; o &quot;Pendiente de confirmación&quot;
          </p>
          <button
            onClick={onCerrar}
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100]">
      <div className="bg-white dark:bg-[#101f28] rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto border border-slate-200 dark:border-slate-600">
        
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-600">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Editar Reserva</h3>
          <button onClick={onCerrar} className="text-slate-400 hover:text-slate-600 dark:hover:text-white transition-colors duration-200">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{reserva.nombreCliente}</h2>
          ...
          <form onSubmit={(e) => { e.preventDefault(); guardarCambios(); }}>
            <div className="bg-slate-50 dark:bg-white/5 rounded-lg p-4 border border-transparent dark:border-slate-600/50">
              <h4 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Información de la Reserva</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:focus:ring-offset-[#101f28]"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-700/80 border border-slate-300 dark:border-slate-600 rounded-md text-slate-700 dark:text-slate-200">
            ...
            <div className="flex justify-end space-x-3 pt-6">
              ...
              <button
                type="submit"
                className="px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 dark:focus:ring-offset-[#101f28] transition-colors duration-200"
              >
                Guardar Cambios
              </button>
            </div>
